...
export default function MagazineChatbotSection() {
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await fetch('/api/magazine-chat', {
        method: 'POST',
  ...
  return (
    <section id="magazine-faq" className="py-20 bg-gray-50 px-6 lg:px-16">
      <div className="max-w-4xl mx-auto">
        {/* Header */}
        <div className="text-center mb-8">
          <h2 className="text-4xl font-bold mb-4">
            Kérdezzen az <span className="text-burgundy">MIT Sloan Management Review</span> magazinról
          </h2>
          <p className="text-gray-600 text-lg">
            Használja AI asszisztensünket a magazinnal kapcsolatos kérdéseire
          </p>
        </div>
          ...
          <div className="h-[500px] overflow-y-auto p-6 space-y-4 bg-gray-50">
            {messages.length === 0 && (
              <div className="text-center text-gray-500 mt-32">
                ...
                <p className="text-gray-600">Kérdezzen bármit az MIT Sloan Management Review magazinról</p>
                <div className="mt-6 flex flex-wrap justify-center gap-2">
                  <button
                    onClick={() => setInput('Milyen témákkal foglalkozik a magazin?')}
                  ...
                  >
                    Milyen témákkal foglalkozik a magazin?
                  </button>
                  <button
                    onClick={() => setInput('Mennyibe kerül az előfizetés?')}
                    className="px-4 py-2 bg-white border border-gray-300 rounded-full text-sm hover:border-orange hover:text-orange transition-colors"
                  >
                    Mennyibe kerül az előfizetés?
                  </button>
                  <button
                    onClick={() => setInput('Milyen gyakran jelenik meg?')}
                    className="px-4 py-2 bg-white border border-gray-300 rounded-full text-sm hover:border-orange hover:text-orange transition-colors"
                  >
                    ...
                    <div className="mt-3 pt-3 border-t border-gray-200">
                      <div className="flex items-start gap-2">
                        ...
                        <div className="text-xs text-gray-600">
                          <p className="font-semibold mb-1">Kapcsolódó információk:</p>
